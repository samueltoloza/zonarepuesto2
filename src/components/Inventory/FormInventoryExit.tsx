...
import { registerSalida } from '@/app/protected/inventory/services/inventory.services'
...
export default function FormInventoryExit({ idUser }: { idUser: string }) {
    ...
    const handleSubmit = async (values: z.infer<typeof formInventoryEntrySchema>) => {
        ...
        try {
            const data = await registerSalida(payload);

            toast.success("Salida registrada", {
                description: `El stock actual es ${data?.stockActual}`
            });

            console.log('Stock actual (salida):', data?.stockActual);
            ...
            toast.error(message || 'Error de red al registrar la salida');
        }
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <PopoverTrigger asChild>
                                        <FormControl>
                                            <Button
                                                ...
                                                role="combobox"
                                                className={cn(
                                                    "justify-between w-full truncate max-w-full",
                    ...
                    <div className="flex justify-center pt-4 col-span-2">
                        <Button type="submit" className="w-full md:w-auto" disabled={form.formState.isSubmitting}>
                            {form.formState.isSubmitting ? 'Enviando...' : 'Registrar Salida'}
                        </Button>
                    </div>
                </div>
            </form>
